create type if not exists public.user_role as enum (
  'super_admin', 'league_owner', 'team_owner', 'coach', 'moderator',
  'camera_operator', 'commentator', 'viewer', 'sponsor'
);

create type if not exists public.onboarding_status as enum (
  'draft', 'pending_verification', 'pending_payment', 'pending_approval',
...
create table if not exists public.team_invites (
  ...
  email text,
  role public.user_role not null default 'team_owner',
...
for select using (id = auth.uid() or public.current_profile_role() = 'super_admin');
create policy "profiles_insert_own" on public.profiles
for insert with check (id = auth.uid());
create policy "profiles_update_own_or_admin" on public.profiles
for update using (id = auth.uid() or public.current_profile_role() = 'super_admin');

create policy "league_owner_or_admin" on public.league_registrations
for all using (owner_id = auth.uid() or public.current_profile_role() = 'super_admin')
with check (owner_id = auth.uid() or public.current_profile_role() = 'super_admin');

create policy "team_invites_league_owner_or_admin" on public.team_invites
for all using (
  public.current_profile_role() = 'super_admin'
  or exists (select 1 from public.league_registrations lr where lr.id = league_registration_id and lr.owner_id = auth.uid())
)
with check (
  public.current_profile_role() = 'super_admin'
  or exists (select 1 from public.league_registrations lr where lr.id = league_registration_id and lr.owner_id = auth.uid())
...
for all using (
  public.current_profile_role() = 'super_admin'
  or exists (select 1 from public.team_registrations tr where tr.id = team_registration_id and tr.owner_id = auth.uid())
  or exists (
    select 1 from public.team_registrations tr
    join public.league_registrations lr on lr.id = tr.league_registration_id
    where tr.id = team_registration_id and lr.owner_id = auth.uid()
  )
)
with check (
  public.current_profile_role() = 'super_admin'
